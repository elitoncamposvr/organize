            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="w-full pb-1.5 inline-flex justify-end">
                        <x-link-btn :href="route('folderarchives.create')">{{ __('Nova Pasta') }}</x-link-btn>
                    </div>

                    @if(count($folders) > 0)
                    <div class="w-full inline-flex border-b py-1.5">
                        <div class="w-3/12">
                            Nome da Pasta/Departamento
                        </div>
                        <div class="w-8/12">
                            Descrição
                        </div>
                    ...
                    @foreach($folders as $folder)
                        <div class="w-full inline-flex border-b my-4 dark:border-b-gray-700">
                            ...
                            <div class="w-1/12 flex items-center justify-evenly">
                                <a href="{{ route('folderarchives.edit', [$folder->id]) }}">
                                ...
                                <form method="post" class="form-destroy" action="{{ route('folderarchives.destroy') }}">
                                    @csrf
                                    @method('delete')
