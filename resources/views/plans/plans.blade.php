            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="w-full pb-1.5 inline-flex justify-end">
                        <x-link-btn :href="route('plans.create')">{{ __('Novo Plano') }}</x-link-btn>
                    </div>
                    ...
                    @foreach($plans as $plan)
                        <div class="w-full inline-flex border-b my-4 dark:border-b-gray-700">
                            ...
                            <div class="w-2/12">
                                {{ $plan->expiration }} dia(s)
                            </div>
                            <div class="w-1/12">
                                {{ $plan->period_deficiency }} mes(es)
                            </div>
                            <div class="w-6/12">
                                {{ $plan->description }}
                            </div>
                            <div class="w-1/12 flex items-center justify-evenly">
                                ...
                                <form method="post" class="form-destroy" action="{{ route('plans.destroy') }}">
                                    @csrf
                                    @method('delete')
